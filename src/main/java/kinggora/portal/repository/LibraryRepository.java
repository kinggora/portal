...
import kinggora.portal.mapper.LibraryMapper;
...
public class LibraryRepository {
    ...
    public int savePost(LibraryPost post) {
        if(mapper.savePost(post) != 1){
            log.error("fail LibraryRepository.savePost");
        }
        return post.getId();
    }
     ...
     * @return 게시글 정보
     */
    public Optional<LibraryPost> findPostById(Integer id) {
    ...
    public void hitUp(Integer id) {
        if(mapper.hitUp(id) != 1) {
            log.error("fail LibraryRepository.hitUp");
        }
    }
    ...
    public void updatePost(LibraryPost post) {
        if(mapper.updatePost(post) != 1) {
            log.error("fail LibraryRepository.updatePost");
        }
    }
    ...
    public void deletePost(Integer id) {
        if(mapper.deletePost(id) != 1) {
            log.error("fail LibraryRepository.deletePost");
        }
    }
}
